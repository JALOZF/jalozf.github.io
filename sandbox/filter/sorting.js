...
// Alphabetical sort, case sensitive. i.e. Will put B before c.
const simpleSort = simpleList.sort()
console.log('First sort', simpleSort)

// Fix for above to make sure that all in list are lowercase.
const lowerList = simpleList.map((item) => item.toLowerCase())
const lowerSort = lowerList.sort()
console.log('Sort fix', lowerSort)


// Same as above. Will display it with the case used, but will do it in alphabetical. To reverse it, switch the output. NOTE: Sort will mess with original the list order. If you need to save a copy of the original list, create a seperate variable. (Modify this to sort by date plus this version on my site.)
...
console.log('See, it edits the list', simpleList)

// Filtering a list. Modify this for the language tags on my site.
function searchList(list, query) {
    function searchCallback(obj) {
        return (
            obj.name.toLowerCase().includes(query.toLowerCase()) ||
            obj.description.toLowerCase().includes(query.toLowerCase()) ||
            obj.tags.find((string) => string.toLowerCase().includes(query.toLowerCase()))
        )
    }
...
// The line immediately below will have problems with the modified search. The original search is below.
// console.log('Search: b', searchList(simpleList, "b"));
...
console.log('Searching: od', searchList(hikes, 'od'))

// Below searches for a name attached to an object
// function searchList(list, query) {
//     function searchCallback(obj) {
